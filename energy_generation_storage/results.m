...
hold on
grid on
plot(airI(1:120),'LineWidth',2);
plot(methI(1:120),'LineWidth',2);
plot(tempI(1:120),'LineWidth',2);
plot(micI(1:120),'LineWidth',2);
plot(mcI(1:120),'LineWidth',2);
plot(zbI(1:120),'LineWidth',2);
ylabel('Current [A]');
xlabel('Time [s]');
legend('Air Qual.','Methane','Temp.','Mic.', 'MC', 'ZigBee');
hold off
...
hold on
grid on
plot(battVoltage,'LineWidth',1.5);
yyaxis left
ylim([2.5 4.2]);
ylabel('Voltage [V]');
yyaxis right
...
xlabel('Time [s]');
xlim([0 1200000]);
plot(battSOC,'LineWidth',1.5);
legend('Batt. Voltage', 'Batt. SOC');
title('Battery Characteristics');
hold off
subplot(1,2,2);
hold on
grid on
plot(battVoltage(1:900).*battCurrent(1:900),'LineWidth',2);
plot(3.3*loadCurrent(1:900),'LineWidth',2);
plot(PVcurrent(1:900).*PVvoltage(1:900),'LineWidth',2);
y1 = mean(3.3*loadCurrent);
y2 = mean(battVoltage.*battCurrent);
y3 = mean(PVcurrent.*PVvoltage);
yline(y1,'-.k','LineWidth',2);
yline(y2,':k','LineWidth',2);
yline(y3,'k','LineWidth',2);
ylabel('Power [W]');
xlabel('Time [s]');
legend('Battery', 'Load', 'PV Cell','Avg. Load', 'Avg. Batt.', 'Avg. PV cell');
hold off

figure('Renderer', 'painters', 'Position', [10 10 1400 400])
hold on
grid on
plot(PVcurrent.*PVvoltage,'LineWidth',2);
plot(PVcurrent.*PVvoltage.*efficiencyPV,'LineWidth',2);
ylabel('Power [W]');
xlabel('Time [s]');
legend('Before Conv.', 'After Conv.');
hold off
saveas(gcf, 'gcurve.png');
