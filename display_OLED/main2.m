...
xlabel({'Original';("P = "+Ppanel(idx)+"W")});
for i = 1:1:5
    subplot(1,6,i+1);
...
for i = 1:1:5
    ...
    xlabel({("DVS("+(100-i*5)+"%)");...
        ...
        ("Savings = "+contrastSavings(idx,6-i))});
end
...
for i = 1:1:5
    ...
    xlabel({("DVS("+(100-i*5)+"%)");...
        ...
        ("Savings = "+concurrentSavings(idx,6-i))});
end
...
xlabel('Concurrent Compensation');
hold off;
...
subplot(2,2,1);
imshow(originalImages{idx});
xlabel('Saturated Image');
subplot(2,2,2);
imshow(brightImages{idx});
xlabel({('Brightness Compensation');("Savings = "+brightSavings(idx))});
subplot(2,2,3);
imshow(contrastImages{idx});
xlabel({('Contrast Enhancement');("Savings = "+contrastSavings(idx))});
subplot(2,2,4);
imshow(concurrentImages{idx});
xlabel({('Concurrent Compensation');("Savings = "+concurrentSavings(idx))});
hold off;
